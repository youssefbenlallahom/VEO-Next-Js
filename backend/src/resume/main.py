...
from ..resume.crew import Resume
from ..resume.skill_extraction import extract_skills_from_pdf  # <-- Import the skill extraction function
import sys
import os
...
def bulk_recommend(data: BulkRecommendRequest, db: Session = Depends(get_db)):
    ...
    if data.job_title:
        query = query.filter(CandidateReport.applied_job_title == data.job_title)
    reports = query.all()
    for r in reports:
...
def list_recommended(limit: int = 50, job_title: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    if job_title:
        # Exact match to avoid mixing similar titles
        query = query.filter(CandidateReport.applied_job_title == job_title)
