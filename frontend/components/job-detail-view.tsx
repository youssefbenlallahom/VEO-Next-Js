...
export function JobDetailView({ jobId }: JobDetailViewProps) {
  ...
  const runCrossJobRecommendations = async () => {
    ...
    try {
      ...
      // Materialize recommended CV copies under current job's recommended folder
      try {
        const matRes = await fetch('/api/materialize-recommended', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            job_title: job.title,
            candidates: matchedCandidates.map(mc => ({ name: mc.name, resumeUrl: mc.resumeUrl }))
          })
        })
        try {
          const body = await matRes.json()
          console.log('🗂️ Materialize result:', body)
        } catch {}
      } catch (e) {
        console.warn('⚠️ Failed to materialize recommended CVs:', e)
      }
    } catch (e) {
  ...
  const getCVUrl = (candidate: any) => {
    ...
    // Derive filename candidates
    const fromResume = candidate?.resumeUrl ? String(candidate.resumeUrl).split('/').pop() : null
    // If candidate is recommended/added, prioritize the actual resume filename to match materialized copy
    const fileName = (candidate.isRecommended || candidate.isAddedFromSuggestions)
      ? (fromResume || nameToFileMap[candidate.name] || (candidate.name.toLowerCase().replace(/\s+/g, '-') + '-cv.pdf'))
      : (nameToFileMap[candidate.name] || fromResume || (candidate.name.toLowerCase().replace(/\s+/g, '-') + '-cv.pdf'))

    // If candidate is recommended or added from suggestions, prefer current job's recommended folder
    if (candidate.isRecommended || candidate.isAddedFromSuggestions) {
      const recommendedUrl = `/api/cv/${encodeURIComponent(job?.title || '')}/recommended/${fileName}`
      console.log('Generated Recommended CV URL:', recommendedUrl)
      return recommendedUrl
    }

    // Otherwise, if resumeUrl is provided, normalize into /api/cv route
    if (candidate.resumeUrl) {
      const safe = candidate.resumeUrl.startsWith('/api/cv/')
        ? candidate.resumeUrl
        : `/api/cv/${encodeURIComponent(candidate.position || job?.title || '')}/${fromResume || fileName}`
      return safe
    }

    // Fallback: use candidate.position if different, else current job
